...
from sde_lib import VESDE, VPSDE, HFS_SDE
...
def get_sde_loss_fn(
    ...
    def loss_fn(model, batch):
        ...
        if isinstance(sde, HFS_SDE):
            mean, std_coeff = sde.marginal_prob(batch, t)
            M_hat_z = (
                c2r(ifft2c_2d((1.0 - mask) * fft2c_2d(r2c(z))))
                .type(torch.FloatTensor)
                .to(config.device)
            )

            perturbed_data = mean + std_coeff[:, None, None, None] * M_hat_z
        else:
        ...
        if not likelihood_weighting:
            if isinstance(sde, HFS_SDE):
                M_hat_score = std_coeff[:, None, None, None] * c2r(
                    ifft2c_2d((1.0 - mask) * fft2c_2d(r2c(score)))
                ).type(torch.FloatTensor).to(config.device)
                losses = torch.square(M_hat_score + M_hat_z)
            else:
